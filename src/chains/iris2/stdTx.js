...
const StdFee = Root.iris2.StdFee;
const Coin = Root.iris2.Coin;
...
class StdTx {
    ...
    SetSignature(sig) {
        ...
        let signature = new Root.iris2.StdSignature({
            pubKey: sig.pub_key,
    ...
    SetPubKey(pubkey) {
        if (Codec.Bech32.isBech32(Config.iris2.bech32.accPub, pubkey)) {
    ...
    GetData() {
        ...
        if (this.signatures) {
            this.signatures.forEach(function (sig) {
                ...
                signatures.push({
                    pub_key: Amino.MarshalJSON(Config.iris2.amino.pubKey, publicKey),
                    signature: signature,
                })
            });
        }
